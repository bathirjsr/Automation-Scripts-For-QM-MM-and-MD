...
if [ "${type}" = "0" ]; then
 ...
 do
 awk -v i="${i}" '$1 ~ i {print $0}' "${acceptor}" | sort -n >> Hbond_analysis_sub_"${current_time}".dat
 awk -v i="${i}" '$2 ~ i {print $0}' "${donor}" | sort -n >> Hbond_analysis_sub_"${current_time}".dat
 done
...
rm hbond_sum_sub.dat
 ...
 for i in ${residues};
 do
 awk -v i="${i}" '$1 ~ i  {print $0}' "${acceptor}" | sort -n >> Hbond_analysis_"${current_time}".dat
 awk -v i="${i}" '$2 ~ i  {print $0}' "${donor}" | sort -n >> Hbond_analysis_"${current_time}".dat
 done
