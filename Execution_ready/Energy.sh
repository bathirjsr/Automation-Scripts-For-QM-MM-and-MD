...
ZPE=$(grep 'total ZPE' Frequency/*_Freq.log|awk '{printf "%7.5f", $(NF-1)}'| awk '{printf "%5.12f", $1/(1000*4.184*627.5095)}')
...
printf "%s\t%s\t%s\n" "${B1}" "${B2}" "${ZPE_KJ}" > "$1"_energy.csv
cat "$1"_energy.csv 
xclip -selection clipboard < "$1"_energy.csv
