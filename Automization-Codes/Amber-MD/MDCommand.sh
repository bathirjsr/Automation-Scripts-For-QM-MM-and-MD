...
Relaxation 1
 &cntrl
     ...
     ntb=1,cut=10.0, ! NVT, 10 A cutoff
     ntp=0, ! No barostat
     ntc=2, ntf=2 ! Shake on
     ntt=3, gamma_ln=1.0, ! Langevin Thermostat, 1.0ps-1
     tempi=0.0, ! Initial Temperature of 0K
     temp0=300.0, ! Final Temperature of 300K
     nstlim=100000, dt=0.001,! 100K steps x 1fs = 100ps
     ...
     ntr=1, restraint_wt=50.0, ! Restrain backbone atoms with 50.0 KCal/Mol/A
...
&cntrl
   ...
   ntb=2,cut=10.0, ! NPT, 10 A cutoff
   ntp=1, pres0=1.0, ! Anisotropic pressure scaling at 1 atm
   ntc=2, ! Shake on
   ntf=2,
   ntt=3, temp0=300.0, gamma_ln=1.0, ! Langevin Thermostat, 1.0ps-1 at 300K
   nstlim=500000, dt=0.002, ! 500K steps x 2fs = 1000ps = 1ns
   iwrap=1, ioutfm=1,
...
protein: equilibration
 &cntrl
	...
	ntt    = 3,
	gamma_ln = 2.0,
	nstlim = 1500000, dt = 0.002,ioutfm=1, ! 1500K steps x 2fs = 3000ps = 3ns
...
cp "${input}".in "${input}"/.
 $do_parallel -i "${input}"/"${input}".in -o "${input}"/"${input}".out -p "${prmtop}" -c "${coords}" -r "${input}"/"${input}".rst -x "${input}"/"${input}".nc -ref "$coords" -O&
...
cp "${input}".in "${input}"/.
 $do_parallel -i "${input}"/"${input}".in -o "${input}"/"${input}".out -p "${prmtop}" -c ${coords}/${coords}.rst -r "${input}"/"${input}".rst -x "${input}"/"${input}".nc -ref ${coords}/${coords}.rst -O&
...
export CUDA_VISIBLE_DEVICES=$CUDA
 $do_gpu -i "${input}"/"${input}".in -o "${input}"/"${input}".out -p "${prmtop}" -c "${min}"/"${min}".rst -r "${input}"/"${input}".rst -x "${input}"/"${input}".nc -ref "${min}"/"${min}".rst -O&
